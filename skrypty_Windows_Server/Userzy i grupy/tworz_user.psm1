...
Każda informacja poza pierwszą musi być poprzedzona Enterem. Działa jedynie dla Takich parametrów jak powyżej. Między użytkownikami musi być pozostawiona linijka przerwy

.Parameter Plik
Tu podaj ścieżkę do pliku
.Example
tworz_user -plik C:\plik.txt
...
{
 $folder = $pth_to_groups_folders + $nazwausera
 mkdir $folder
 $prawo= $nazwausera + ":(RX)"
     
    icacls $folder /grant:r "$prawo"  /t
}
     }
     New-AdUser -Name $nazwausera -Accountpassword  (ConvertTo-SecureString $haslo -AsPlainText -Force) -CannotChangePassword $True -Description $opis -EmailAddress $poczta -GivenName $imie -Surname $nazwisko -MobilePhone $telefon -UserPrincipalName ($nazwausera +".@inzynierka.local") -enabled $True 
     if ($grupa -eq "szef") 
     {
